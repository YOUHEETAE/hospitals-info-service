...
public class HospitalDetailApiServiceImpl implements HospitalDetailApiService {
    ...
    private final HospitalDetailAsyncRunner asyncRunner;           // 전문의 API 비동기 실행기
    private final HospitalDetailRepository hospitalDetailRepository;       // 전문의 정보 저장소 (JPA)
    ...
    /**
     * ✅ 병원 전체를 대상으로 API 호출 후 전문의 데이터 비동기 저장 실행
     * 1. 기존 전문의 데이터 모두 삭제
     * 2. 전체 병원코드 가져오기
